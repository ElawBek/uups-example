...
import { deployVersionOneFixture, deployVersionTwoFixture } from "./fixtures";

import { signERC2612Permit } from "eth-permit";

import { VaultV1, VaultV2, Token, VaultV2__factory } from "../typechain-types";
...
describe("UUPS", () => {
  ...
  describe("Version 1", () => {
    ...
    beforeEach(async () => {
      ({ owner, alice, token0, token1, vaultV1Proxy, implV1 } =
        await loadFixture(deployVersionOneFixture));
    });

    describe("Deployment", () => {
      it("Proxy state", async () => {
        expect([
          await vaultV1Proxy.owner(),
          await vaultV1Proxy.name(),
          await vaultV1Proxy.token(),
          await vaultV1Proxy.minAmount(),
        ]).to.deep.eq([
          owner.address,
          "Vault",
          token0.address,
      ...
      it("Origin implementation state", async () => {
        expect([
          await implV1.owner(),
          await implV1.name(),
          await implV1.token(),
      ...
      it("Attempt to initialize the original contract should revert", async () => {
        await expect(
          implV1.initialize(token0.address, "Error")
  ...
  describe("Version 2", () => {
    ...
    beforeEach(async () => {
      ({ owner, alice, token0, token1, token2, vaultV2Proxy, implV2 } =
        await loadFixture(deployVersionTwoFixture));
    });
